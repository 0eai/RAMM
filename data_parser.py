...
def load_perception_subject_new(features: List[str], subject_id, normalizers, label_dim) -> Tuple[List[np.ndarray], np.ndarray, np.ndarray]:
    """
    Loads data for a single subject for the perception task
    ...
    method, so no need to take care of that. It just needs to be called in the load_mimic_subject method somewhere
    to normalize the features
    :param label_dim: the dimension of the label to be used
    :return: features, labels, metas.
        Assuming every subject consists of n segments of lengths l_1,...,l_n:
            features is a list (length n) of ndarrays of shape (l_i, feature_dim)  - each item corresponding to a segment
            labels is a ndarray of shape (n, num_classes) (labels for each element in the features list, assuming every segment
    ...
    # Load features from multiple files
    features_list = []
    for feature in features:
        ...
        feature_idx = 2  # Assuming features start from the 3rd column
        feature_values = feature_df.iloc[:, feature_idx:].values

        if normalizers and normalizers[feature] is not None:
            feature_values = normalizers[feature].transform(feature_values)
    ...
    return features_list, label, metas


def load_humor_subject(features: List[str], subject_id: str, normalizers: Optional[Dict[str, StandardScaler]]) -> Tuple[List[List[np.ndarray]], np.ndarray, np.ndarray]:
    # Parse labels
    label_path = PATH_TO_LABELS[HUMOR]
    label_files = sorted(glob(os.path.join(label_path, subject_id + '/*.csv')))
    assert len(label_files) > 0, f'Error: no available humor label files for coach "{subject_id}": "{label_files}".'
    ...
    for feature in features:
        feature_path = os.path.join(PATH_TO_FEATURES[HUMOR], feature)
        feature_files = sorted(glob(os.path.join(feature_path, subject_id + '/*.csv')))
        assert len(feature_files) > 0, f'Error: no available "{feature}" feature files for coach "{subject_id}": "{feature_files}".'
        
        feature_df = pd.concat([pd.read_csv(feature_file) for feature_file in feature_files])
        
        if normalizers and normalizers.get(feature) is not None:
            feature_values = feature_df.iloc[:, feature_idx:].values
            feature_df.iloc[:, feature_idx:] = normalizers[feature].transform(feature_values)
        
        feature_data_list.append(feature_df)

    features_list = [[] for _ in range(len(label_df))]

    for i, (_, y) in enumerate(label_df.iterrows()):
        start = y['timestamp_start']
        end = y['timestamp_end']
        segment_id = y['segment_id']

        for feature_df in feature_data_list:
            segment_features = feature_df[feature_df.segment_id == segment_id]
            label_features = segment_features[(segment_features.timestamp >= start) &
                                              (segment_features.timestamp < end)].iloc[:, feature_idx:].values
            # Imputation
            if label_features.shape[0] == 0:
                label_features = np.zeros((1, feature_df.shape[1] - feature_idx))
            features_list[i].append(label_features)
...
def load_data(task:str,
    ...
    feature_str = "_".join([feature.replace(os.path.sep, "-") for feature in features])
    data_file_name = f'data_{task}_{feature_str}_{label_dim + "_" if len(label_dim) > 0 else ""}_{"norm_" if normalize else ""}' \
    ...
    data = {'train': {'features': [], 'label': [], 'meta': []},
            'devel': {'features': [], 'label': [], 'meta': []},
            'test': {'features': [], 'label': [], 'meta': []}}
    ...
    if not(normalize is None):
        if type(normalize) == bool:
            for feature in features:
                normalizers[feature] = fit_normalizer(task=task, feature=feature) if normalize else None
        else:
    ...
    for partition, subject_ids in partition2subject.items():
        ...
        for subject_id in tqdm(subject_ids):
            if task == HUMOR:
                ...
                features_list, labels, metas = load_humor_subject(features=features, subject_id=subject_id,
                                                                      normalizers=normalizers)
            elif task == PERCEPTION:
                features_list, labels, metas = load_perception_subject_new(features=features, subject_id=subject_id,
                                                                           normalizers=normalizers, label_dim=label_dim)

            # print(len(features_list[0][0]), type(features_list[0][0]), len(labels))
            data[partition]['features'].extend(features_list)
