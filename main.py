...
from model import UnimodalModel, MultimodalModel
...
def parse_args():
    ...
    parser.add_argument('--features', type=str, nargs='+', default=["vit-fer"],
                        help='Specify the features.')
    ...
    parser.add_argument('--wandb', action='store_true',
                        help='Specify whether to log experiment results to wandb (default: False).')
...
def main(args):
    ...
    data = load_data(args.task, args.paths, args.features, args.label_dim, args.normalize, save=args.cache)
    ...
    if args.eval_model is None:  # Train and validate for each seed
        ...
        val_losses, val_scores, best_model_files, test_scores, train_logs = [], [], [], [], []
        ...
        for seed in seeds:
            ...
            val_loss, val_score, best_model_file, train_log = train_model(args.task, model, data_loader, args.epochs,
            ...
            train_logs.append(train_log) ## TODO log metrics
        ...
        if args.wandb:
            wandb.log({'seed': seeds[best_idx]}) ## TODO log metrics
            for metrics in train_logs[best_idx]: ## TODO log metrics
                wandb.log(metrics)               ## TODO log metrics
    ...
    if args.predict:  # Make predictions for the test partition; this option is set if there are no test labels
        ...
        best_model = torch.load(model_file, map_location=args.device)
...
if __name__ == '__main__':
    ...
    if args.wandb:
        ...
        wandb.init(dir=config.WANDB_PATH)
    ...
    if type(list()) == type(args.features):
        if '+' in args.features[0]:
            args.features = [feature.strip() for feature in args.features[0].split('+')]
    else:
        if '+' in args.features:
            args.features = [feature.strip() for feature in args.features.split('+')]
        else:
    ...
    args.log_file_name =  '{}_{}_[{}]_[{}_{}_{}_{}]_[{}_{}]'.format(args.encoder, datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), feature_str,
                                                args.model_dim, args.encoder_n_layers, args.nhead, args.d_fc_out, args.lr,args.batch_size)
